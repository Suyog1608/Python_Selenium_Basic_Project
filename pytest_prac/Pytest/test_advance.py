...
def test_tc08_VerifyLogoAdvance(browser):
    assert browser.find_element(By.CSS_SELECTOR, "img[src*= 'login_left.gif']").is_displayed() == True
    time.sleep(2)


def test_Tc09_verifyLoginWithInvalidCreds(browser):
    driver = browser
...
def test_Tc091_verifyLoginWithInvalidCreds(username, pwd, browser):
    driver = browser
...
def test_Tc010_verifyLoginWithValidCreds(browser):
    driver = browser
...
def test_Tc011_verifyLogout(browser):
    driver = browser
    driver.find_element(By.XPATH, "//input[@name='user_name']").send_keys("admin")
    driver.find_element(By.XPATH, "//input[@name='user_password']").send_keys("admin")
    driver.find_element(By.XPATH, "//input[@name='Login']").click()
